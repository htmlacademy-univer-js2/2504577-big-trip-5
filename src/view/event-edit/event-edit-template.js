...
function createOfferSelectorTemplate(title, price, checked = false) {
  ...
  return `<div class='event__offer-selector'>
            ...
            <label class='event__offer-label' for='event-offer-${alias}-1' name='${title.replaceAll(' ', '-')}'>
...
function createEventEditTemplate(point, destinations, offers, userAction) {
  ...
  const typeEventTemplate = Object.values(TYPES_EVENT)
    .map((type) =>
      type === pointType
        ? createTypeEventTemplate(type, true)
        : createTypeEventTemplate(type)
    )
    .join('');
  ...
  const offerSelectorTemplate = availableOffers
    .map(({ id, title, price }) =>
      selectedOffers.includes(id)
        ? createOfferSelectorTemplate(title, price, true)
        : createOfferSelectorTemplate(title, price)
    )
    .join('');
  ...
  return `<li class='trip-events__item'>
              <form class='event event--edit' action='#' method='post'>
                <header class='event__header'>
                  ...
                  <button class='event__reset-btn' type='reset'>${userAction === UserAction.UPDATE_POINT ? 'Delete' : 'Cancel'}</button>
